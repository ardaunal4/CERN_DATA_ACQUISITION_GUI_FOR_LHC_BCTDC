...
class Window(QMainWindow): 
    ...
    def histogram(self, step_list, path, variable_name):

        counter = 0

        for step in step_list:

            path_name = path + '_hist_step' + str(counter) + '.png'
            title = variable_name + " Histogram of Step " + str(counter)
            plt.figure(figsize = (10, 8))
            plt.hist(step, bins = 100)
            plt.xlim(min(step), max(step))
            plt.xlabel("Bin", fontsize=25)
    ...
    def analysis(self):
        ...
        for df in dataFrame_list:
            ...
            new_path = path + "/" + data_save_list[counter]

            try:
                os.mkdir(new_path)
            except OSError:
                failed = "Creation of directory failed!"
                self.result_list.addItem(failed) 
            else:
                success = "Directory created successfully!"
                self.result_list.addItem(success)

            name = new_path + '/' + data_save_list[counter]
            ...
            if len(peaks) == 80:
                ...
                self.histogram(step_list, name, data_save_list[counter])

            else: 

                 message = data_save_list[counter] + ' contains error!'
                 info_box = QMessageBox.information(self, "WARNING!", message)
